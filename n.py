...
TARGET_CHANNEL_ID = "-1003107433425"
...
def print_banner():
    banner = f"""
{Colors.CYAN}{'='*60}
{Colors.GREEN}    TELEGRAM BOT FILE PROCESSOR v13.0
    Fixed Archive Extraction | ZIP | RAR | 7Z
...
def ensure_directories():
    ...
    for dir_path in [DOWNLOAD_DIR, EXTRACT_DIR, TEMP_DIR]:
...
def detect_archive_type(file_path: Path) -> Optional[str]:
    ...
    try:
        with open(file_path, 'rb') as f:
            magic = f.read(8)  # Read more bytes for better detection
...
# ==================== ZIP EXTRACTION ====================
def extract_zip(file_path: Path, extract_to: Path, password: str = None) -> bool:
    """Extract ZIP file with optional password"""
    try:
        with zipfile.ZipFile(file_path, 'r') as zip_ref:
            ...
            for idx, file_name in enumerate(file_list):
                try:
                    ...
                    if "Bad password" in str(e) and password:
                ...
                if idx % 10 == 0 or idx == total - 1:
                    ...
                    logger.info(f"ZIP extraction: {percent}%")
            
            logger.info(f"✅ ZIP extracted: {total} files")
            return True
...
# ==================== RAR EXTRACTION ====================
def extract_rar(file_path: Path, extract_to: Path, password: str = None) -> bool:
    """Extract RAR file using unar"""
    try:
        # Check if unar is installed
        result = subprocess.run(['which', 'unar'], capture_output=True)
        if result.returncode != 0:
            logger.error("unar not installed. Install: pkg install unar")
            return False
        ...
        if result.returncode == 0:
            logger.info(f"✅ RAR extracted successfully")
            return True
        else:
        ...
        logger.error("RAR extraction timeout")
        return False
    except Exception as e:
...
# ==================== 7Z EXTRACTION ====================
def extract_7z(file_path: Path, extract_to: Path, password: str = None) -> bool:
    """Extract 7Z file using 7z or unar"""
    try:
        # Try 7z first
        result = subprocess.run(['which', '7z'], capture_output=True)
        if result.returncode == 0:
            cmd = ['7z', 'x', str(file_path), f'-o{extract_to}', '-y']
            if password:
                cmd.append(f'-p{password}')
            
            result = subprocess.run(cmd, capture_output=True, timeout=600)
            if result.returncode == 0:
                logger.info(f"✅ 7Z extracted with 7z")
                return True
...
def extract_archive(file_path: Path, extract_to: Path, password: str = None) -> Tuple[bool, List[Path]]:
    ...
    try:
        ...
        if archive_type == 'zip':
            success = extract_zip(file_path, extract_to, password)
        elif archive_type == 'rar':
            success = extract_rar(file_path, extract_to, password)
        elif archive_type == '7z':
            success = extract_7z(file_path, extract_to, password)
        ...
        if success:
            ...
            logger.error(f"Failed to extract {archive_type}")
            return False, []
...
async def process_and_send(bot: Bot, file_path: Path, password: str, file_name: str, status_msg) -> Dict:
    ...
    try:
        ...
        await status_msg.edit_text(
            ...
            f"Type: {detect_archive_type(file_path) or 'Single file'}",
            parse_mode='Markdown'
        )
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE, link: str):
    ...
    async def process():
        try:
            file_path = TEMP_DIR / f"temp_{int(time.time())}"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"**Archive Support:**\n"
        f"📦 **ZIP** - Native Python support\n"
        f"📦 **RAR** - Using unar (requires installation)\n"
        f"📦 **7Z** - Using 7z or unar\n\n"
        f"**Features:**\n"
...
def main():
    ...
    result = subprocess.run(['which', 'unar'], capture_output=True)
    if result.returncode != 0:
        print(f"{Colors.YELLOW}⚠ Warning: unar not installed. RAR/7Z extraction may fail!{Colors.END}")
        print(f"{Colors.YELLOW}Install: pkg install unar (Termux) or sudo apt install unar (Linux){Colors.END}")
    ...
    print(f"{Colors.CYAN}✓ Archive support: ZIP (native), RAR/7Z (unar){Colors.END}")
