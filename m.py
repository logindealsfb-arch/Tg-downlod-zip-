...
# User ID of allowed users (optional - leave empty to allow anyone)
ALLOWED_USERS = []  # Add user IDs like [123456789] or leave empty for anyone
...
def print_banner():
    banner = f"""
{Colors.CYAN}{'='*60}
{Colors.BOLD}    TELEGRAM BOT FILE PROCESSOR v6.0 (Queue Enabled)
    Send files to bot | Bot processes & sends to channel
    Supports: ZIP | RAR | 7Z | Images | Videos
    Multiple files will be queued and processed sequentially
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text(
        ...
        f"📁 {file_name}\n"
        f"💾 {size_mb:.1f} MB\n"
        f"⏳ Position in queue: {task_queue.queue.qsize() + 1}",
        parse_mode='Markdown'
    )
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def process():
        try:
            # Send video directly using file_id (no need to re-upload)
            await context.bot.send_video(
                chat_id=TARGET_CHANNEL_ID,
                video=video.file_id,
                caption=f"🎥 {file_name}"
            )
            await status_msg.edit_text(f"✅ Video forwarded: {file_name}")
        except Exception as e:
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def process():
        try:
            ...
            await status_msg.edit_text("✅ Photo sent to channel!")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_msg = (
        ...
        "3️⃣ Send all images/videos to the target channel\n\n"
        "**Multiple files?**\n"
        "They will be queued and processed one by one.\n\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_msg = (
        ...
        "/status - Queue status\n\n"
        "**How to use:**\n"
        "• Send any file (document, photo, video)\n"
        "• If you send multiple, they are queued\n"
        "• Bot processes them sequentially\n\n"
        "**Supported archives:**\n"
        "ZIP, RAR, 7Z (password: cosplaytele)\n\n"
        "**Media handling:**\n"
        "• All media sent as documents (original quality)\n"
...
def main():
    ...
    if TELEGRAM_BOT_TOKEN == "YOUR_BOT_TOKEN_HERE":
        print(f"{Colors.FAIL}✗ Please add your bot token in the script!{Colors.END}")
        return
    ...
    print(f"{Colors.CYAN}Queue system active. Send multiple files, they'll be processed sequentially.{Colors.END}")
